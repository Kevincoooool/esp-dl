...
#define CAMERA_SENSOR_TYPE           "DVP"
#elif CONFIG_EXAMPLE_ENABLE_MIPI_CSI_CAM_SENSOR
#define CAM_DEV_PATH                 ESP_VIDEO_MIPI_CSI_DEVICE_NAME
#define CAMERA_SENSOR_TYPE           "MIPI CSI"
#else
#error "未选择摄像头传感器类型，请在配置中启用MIPI CSI或DVP传感器"
#endif
...
static int ksdiy_video_open(char *dev, ksdiy_fmt_t init_fmt)
{
    ...
    ESP_LOGI(TAG, "使用%s摄像头传感器", CAMERA_SENSOR_TYPE);
    ...
    ESP_LOGI(TAG, "默认分辨率: 宽度=%" PRIu32 " 高度=%" PRIu32, default_format.fmt.pix.width, default_format.fmt.pix.height);
    ...
    if (default_format.fmt.pix.pixelformat != init_fmt) {
        ...
        ESP_LOGI(TAG, "成功设置像素格式为: 0x%08lx", (uint32_t)init_fmt);
    } else {
        ESP_LOGI(TAG, "使用默认像素格式: 0x%08lx", (uint32_t)init_fmt);
    }
...
static jpeg_enc_input_format_t get_jpeg_enc_input_fmt(uint32_t video_fmt)
{
    ...
    case KSDIY_VIDEO_FMT_RGB888:
        ...
        break;
    default:
        ESP_LOGE(TAG, "不支持的格式: 0x%08lx", (uint32_t)video_fmt);
        ret_fmt = -1;
        break;
    }
    return ret_fmt;
...
esp_err_t ksdiy_camera_init(ksdiy_fmt_t fmt)
{
    ...
    // 显示传感器配置信息
#if CONFIG_EXAMPLE_ENABLE_DVP_CAM_SENSOR && CONFIG_EXAMPLE_ENABLE_MIPI_CSI_CAM_SENSOR
    ESP_LOGI(TAG, "检测到同时启用了DVP和MIPI CSI传感器，优先使用DVP传感器");
#endif
    ESP_LOGI(TAG, "传感器类型: %s", CAMERA_SENSOR_TYPE);
    ESP_LOGI(TAG, "设备路径: %s", CAM_DEV_PATH);
    
    // 初始化摄像头硬件
    ESP_LOGI(TAG, "初始化摄像头硬件配置...");
    ...
    // 打开摄像头设备
    ESP_LOGI(TAG, "打开摄像头设备: %s", CAM_DEV_PATH);
    ...
    ESP_LOGI(TAG, "摄像头参数: 分辨率=%ldx%ld, 像素格式=0x%08lx", 
    ...
    if (jpeg_enc_infmt == -1) {
        ESP_LOGE(TAG, "不支持的像素格式: 0x%08lx", (uint32_t)format.fmt.pix.pixelformat);
        close(fd);
        free(s_camera);
        s_camera = NULL;
        return ESP_FAIL;
    ...
    ESP_LOGI(TAG, "摄像头初始化成功! 分辨率: %ldx%ld, 传感器类型: %s", 
             s_camera->width, s_camera->height, CAMERA_SENSOR_TYPE);
    
    // 输出配置信息总结
    ESP_LOGI(TAG, "=== 摄像头配置信息总结 ===");
#if CONFIG_EXAMPLE_ENABLE_DVP_CAM_SENSOR
    ESP_LOGI(TAG, "DVP传感器配置:");
    ESP_LOGI(TAG, "  SCCB I2C端口: %d", CONFIG_EXAMPLE_DVP_SCCB_I2C_PORT);
    ESP_LOGI(TAG, "  SCCB SCL引脚: %d", CONFIG_EXAMPLE_DVP_SCCB_I2C_SCL_PIN);
    ESP_LOGI(TAG, "  SCCB SDA引脚: %d", CONFIG_EXAMPLE_DVP_SCCB_I2C_SDA_PIN);
    ESP_LOGI(TAG, "  XCLK频率: %d Hz", CONFIG_EXAMPLE_DVP_XCLK_FREQ);
    ESP_LOGI(TAG, "  XCLK引脚: %d", CONFIG_EXAMPLE_DVP_XCLK_PIN);
#endif
#if CONFIG_EXAMPLE_ENABLE_MIPI_CSI_CAM_SENSOR && !CONFIG_EXAMPLE_ENABLE_DVP_CAM_SENSOR
    ESP_LOGI(TAG, "MIPI CSI传感器配置:");
    ESP_LOGI(TAG, "  SCCB I2C端口: %d", CONFIG_EXAMPLE_MIPI_CSI_SCCB_I2C_PORT);
    ESP_LOGI(TAG, "  SCCB SCL引脚: %d", CONFIG_EXAMPLE_MIPI_CSI_SCCB_I2C_SCL_PIN);
    ESP_LOGI(TAG, "  SCCB SDA引脚: %d", CONFIG_EXAMPLE_MIPI_CSI_SCCB_I2C_SDA_PIN);
#endif
    ESP_LOGI(TAG, "  图像格式: 0x%08lx", s_camera->pixel_format);
    ESP_LOGI(TAG, "  JPEG质量: %d", JPEG_ENC_QUALITY);
    ESP_LOGI(TAG, "=========================");
